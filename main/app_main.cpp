...
extern "C" void app_main() {
    ...
    audio_game_init();          // enregistre toutes les pistes aupres du player
    ...
    while (true) {
        ...
        // Capture d'ecran : MENU maintenu 500 ms (hors combo RUN+MENU).
        // Ecrit un BMP dans /sdcard/AKBRICKS/. Le seuil evite de declencher
        // sur un appui court (MENU court = Options depuis l'ecran-titre).
        static uint32_t menu_hold_start = 0;
        if (k.MENU && !k.RUN) {
            uint32_t now = esp_timer_get_time() / 1000;
            if (!menu_hold_start) {
                menu_hold_start = now;
            } else if (now - menu_hold_start >= 500) {
                menu_hold_start = 0;
                char shot[64];
