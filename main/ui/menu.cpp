...
static void show_controls() {
    ...
    gfx_text(90, 10, T(STR_CONTROLS), color_yellow);
    gfx_text(20, 45,  "Stick / L-R : raquette", color_white);
    gfx_text(20, 68,  "A : lancer / laser",     color_white);
    ...
    gfx_text(20, 185, T(STR_BACK), color_yellow);
    gfx_flush();
    // attend un appui B (front) pour revenir
...
static void draw_menu(int sel, bool in_game) {
    gfx_clear(color_black);
    gfx_text(120, 10, T(STR_MENU), color_yellow);

    char buf[40];
    const int x = 40, y0 = 45, dy = 18;
