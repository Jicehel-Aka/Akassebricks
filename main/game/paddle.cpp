...
void draw_paddle(const Paddle& p) {
    ...
    // Colonne du sprite selon les bonus ACTIFS. La colle initiale (bille posee
    // avant lancement) utilise BONUS_GLUE avec sticky_timer == -1 : ce n'est PAS
    // le bonus colle, on ne montre donc pas le sprite collant a ce moment-la.
    bool glue  = (p.bonus_flags & BONUS_GLUE)  && (p.sticky_timer > 0);
    bool laser = (p.bonus_flags & BONUS_LASER);
    int column = 0;
    if (glue && laser)  column = 3;
    else if (glue)      column = 1;
    else if (laser)     column = 2;
